...
> `v1.0.0` 稳定基线：当前维护版本在不改变双人房间模型的前提下，补充了大图片、剪贴板和 Beta 文件传输。

只允许两位参与者进入的双人加密聊天 MVP。两端使用同一条邀请链接，以完全对等的方式完成 WebRTC 协商；文字、图片、语音和 Beta 文件先在浏览器内使用 AES-GCM 加密，再通过 DataChannel 传输，房间凭证、个人头像和常规密文历史保存在各自设备的 IndexedDB。
...
- WebRTC 点对点数据通道
- AES-GCM 应用层二次加密和安全码
- 文字、语音和最大 100 MB 图片；支持复制文字，以及直接粘贴剪贴板中的文字、图片或文件
- 最大 100 MB 的 Beta 文件传输；文件入口和消息卡片均显示 Beta
- 10 类 Unicode 表情、15 类颜文字和 4 类图片表情包；颜文字可按“常用 / 开心 / 问候 / 可爱 / 情绪 / 动作”等多层目录选择
- 消息框继续支持 `Win + .`、Fn/🌐 等系统原生表情面板，并提供 `Ctrl/⌘ + Shift + 空格` 打开应用内面板
- 大附件按 192 KB 分块独立加密，密文包继续按 12,000 字符分片；DataChannel 使用高低水位背压，避免一次灌满发送缓冲区
- 按 `roomId` 唯一保存完整房间凭证，重新打开首页时恢复全部本机聊天
- 每个已保存房间保持独立信令与 WebRTC 连接；切换侧栏只切换当前界面
- 聊天顺序独立保存，切换不会自动置顶；支持拖动排序、重命名和上传 200 × 200 聊天图标
- 聊天名称和图标作为 AES-GCM 加密控制消息在在线两端同步；并发修改按单调版本号确定同一个结果
- 支持仅本机删除单条消息或整个聊天，不向对方发送删除指令
- 多行聊天输入框可纵向调整高度；工具栏、输入区和己方气泡宽度相互独立
...
- 房间凭证和历史记录只保留在各自设备的 IndexedDB，不会跨设备同步；仅聊天名称和图标会在双方在线时同步。
- 不读取或保存 MAC、deviceId、浏览器指纹；本机恢复只依赖 IndexedDB 中的 `roomId + secret`。
- 双方必须同时在线才能收到实时消息；离线时发送的消息只保留在发送方本机。
- 删除消息或聊天只影响执行操作的浏览器；双方记录不同属于当前产品允许的状态。
- 双方离线时分别修改聊天名称或图标，重连后只保留版本号更高的一份，不提供修改历史或字段级合并。
...
- 当前不支持实时音视频通话。
- 1.5 MB 以内的图片、语音和文件仍进入本机密文历史；更大的图片/文件为当前页面内的流式传输，刷新后不会恢复，以避免浏览器 IndexedDB 配额失控。

正式版本应增加持久化成员公钥、一次性邀请核销、加密离线信箱、密钥恢复、大附件断点续传/校验和多地域 TURN 监控。
