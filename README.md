...
只允许两位参与者进入的双人加密聊天 MVP。两端使用同一条邀请链接，以完全对等的方式完成 WebRTC 协商；文字、图片、语音和视频先在浏览器内使用 AES-GCM 加密，再通过 DataChannel 传输，房间凭证、个人头像和密文历史保存在各自设备的 IndexedDB。
...
- WebRTC 点对点数据通道
- AES-GCM 应用层二次加密和安全码
- 文字、1.5 MB 以内图片/语音和 8 MB 以内视频消息；较大消息按 DataChannel 缓冲区背压分片发送
...
正式版本应增加持久化成员公钥、一次性邀请核销、加密离线信箱、密钥恢复、大文件流式传输进度和多地域 TURN 监控。
