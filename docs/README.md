...
3. [34 万条 Redis 命令之后](project-retrospective.md)：沿着一次真实用量问题，解释固定轮询怎样改成事件唤醒。
...
- [Supabase + Vercel HTTPS 主备信令](signaling-resilience.md)：Supabase 主通道、Redis 冷备、单边故障桥接和验收矩阵。
- [事件唤醒信令与浏览器唯一性 RFC](wake-driven-signaling-rfc.md)：记录已经落地的 v4 Wake、浏览器安装 ID、多标签页单网络负责人、Redis 冷备和容量预算。
...
| 信令 | Supabase Realtime Broadcast 主用；明确故障时短时启用 Vercel `/api/signal` 与 Redis，并把单边故障事件桥接到健康端 |
| 协商协议 | protocol v4；Wake/ACK 采用持久序号和有界重试，所有信令带每房间 P-256 ECDSA 成员签名，epoch + negotiation ID 隔离重连轮次 |
| 应用层加密 | Web Crypto API，随机会话秘密经 SHA-256 导入为 AES-GCM 密钥，每条消息使用独立 12 字节 IV |
| 本机恢复 | IndexedDB 保存房间秘密、成员密钥、密文历史、随机浏览器安装 ID、Wake 序号和网络租约；每个房间最多保留 200 条消息 |
| 双人席位 | 创建者公钥位于邀请 Fragment；首个验签接收者原子占第二席位，此后断线或空房都不允许陌生公钥替换 |
| 多聊天 | 一个浏览器配置目录只让一个标签页维护网络；多个房间共享 Supabase client，切换界面不会释放其他房间连接 |
