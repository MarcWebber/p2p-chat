...
正常聊天界面不显示调试栏。脱敏连接日志仍写入浏览器 Console，统一前缀为 `[twoonly][traceId][时间][阶段][事件代码]`；页面内存最多保留最近 200 条，刷新后清空，也不会持久化到 IndexedDB。
...
完整房间凭证、每房间 P-256 成员密钥和 AES-GCM 密文历史保存在每台设备的 IndexedDB。它能在同一浏览器重新打开后恢复，但不会跨设备同步；双方离线时也没有服务器信箱。成员密钥只证明“这个浏览器拥有这个房间的一个席位”，不是账号或可跨设备恢复的用户身份。

### 5.2 为什么不能公开完整邀请链接？

URL fragment 中包含会话秘密和创建者公钥。fragment 通常不会随 HTTP 请求发送给服务器，但拿到完整链接的人也拿到了应用层秘密；在第二席位尚未确认时，对方还可能抢先发送有效签名并占用这个席位。席位锁定后，单凭链接不能把原成员替换掉，但泄露的秘密仍不应视为安全。请通过可信渠道分享，并在双方页面核对安全码。
...
不能。创建者公钥随邀请发送；首个通过验签的接收者会用 IndexedDB 读写事务原子占用第二席位。两端随后都保存自己的房间私钥材料和对端公钥。第三个成员即使知道 Room ID 和 Secret，其公钥仍不在两席名单里，会收到签名的 `rejected(member-locked)`，不会因为对方待机、锁屏、断线、关闭页面或 PeerLock 超时而替换原成员。

`PeerLock` 仍会处理同一个合法成员的多个页面实例和 epoch：旧页面失败后，同一成员可凭原房间私钥从新页面实例重连。它不再决定“谁能成为成员”。第二席位首次确认前仍存在邀请泄露后的抢占窗口；当前没有服务端一次性邀请核销。

### 5.5 清理站点数据或删除聊天后，还能回到原房间吗？

不能保证，而且对已经锁定的房间通常不能。TwoOnly 不读取 MAC、deviceId 或浏览器指纹，也没有账号恢复。清理站点数据或删除房间会一并删除本机成员私钥；再次打开旧邀请会生成另一把成员密钥，而对端只接受原公钥，因此会把它当作陌生成员。这个限制是持久双人席位的隐私代价，目前没有密钥导出、迁移或重置房间成员的功能。

### 5.6 为什么改头像后，之前的消息也会一起变化？

昵称和头像的最新版本通过 DataChannel 中的 AES-GCM 加密 `profile-metadata` 控制帧同步，并保存在接收端房间记录里。消息列表显示历史时用这份最新资料作为覆盖层，所以旧消息会显示新头像和昵称；历史消息本身的 AES-GCM 密文不会被重写。若两端从未重新连上，离线一端自然不会收到这次资料更新。

升级前的旧消息没有成员公钥字段，因此旧房间会假定全部 `peer` 历史来自升级后锁定的同一位对端。旧版曾允许对端更换；如果某个旧房间确实换过人，这些旧消息可能会被显示成当前锁定成员的最新资料。协议无法从旧密文中恢复缺失的历史身份，这属于迁移兼容取舍；新建 v3 房间不会再发生成员更换。
